...
describe('AuthService', () => {
  ...
  describe('tokenBlock', () => {
    it('should block a token', async () => {
      ...
      expect(cacheManager.set).toHaveBeenCalledWith(`BLOCKED_TOKEN_${token}`, payload, expect.any(Number));
    });
  });
  ...
  describe('authenticate', () => {
    ...
    it('should throw an error for incorrect password', async () => {
      const user = { email: 'test@test.ai', password: 'hashedPassword' };
      jest.spyOn(mockUserRepository, 'findOne').mockResolvedValue(user);
      jest.spyOn(bcrypt, 'compare').mockImplementation((a, b) => false);

      await expect(authService.authenticate('test@test.ai', 'password')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('issueToken', () => {
    ...
    it('should issue an access token', async () => {
      ...
      expect(jwtService.signAsync).toHaveBeenCalledWith({ sub: user.id, type: 'access', role: user.role }, { secret: 'secret', expiresIn: 300 });
    ...
    it('should issue an access token', async () => {
      ...
      expect(jwtService.signAsync).toHaveBeenCalledWith({ sub: user.id, type: 'refresh', role: user.role }, { secret: 'secret', expiresIn: '24h' });
      expect(result).toBe(token);
    });
  });
